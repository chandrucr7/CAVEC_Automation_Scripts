...
Test_case86
    ...
    ${auth}=    Create List    38077824    CW04qVYK7xB0vvhPGm3oLzOHdauFaN42
    Create Session    mysession    ${base_url}    auth=${auth}    verify=true
    ${random_client_ref_num}=    Generate Random String    10    [LETTERS]

    #------------------------------------------ Request endpoint encryption-----------------------------------------------
    ${encrypt_body}=   Evaluate    { "plain_payload": { "mobile": "9790590318", "requested_services": "whatsapp,paytm,amazon,flipkart,facebook,twitter", "client_ref_num": "${random_client_ref_num}" }, "secret_name": "crypto_framework_secret", "secret_key": "digitap_public", "kms_id": "39341d7d-b0d2-4c0a-82a3-1dacfac47d92", "encryption_type": "jwe" }
    ${headers}=    Create Dictionary    Content-Type=application/json
    Log To Console    Encrypt Request Body: ${encrypt_body}

    ${encrypt_response}=    Post Request     mysession     https://svcint.digitap.ai/validation/v1/encrypt   json=${encrypt_body}    headers=${headers}
    ...
    ${decrypt_body}=   Evaluate    { "encrypted_response": "${encrypted_data_output}", "secret_name": "crypto_framework_secret", "secret_key": "38077824_private", "kms_id": "39341d7d-b0d2-4c0a-82a3-1dacfac47d92", "encryption_type": "jwe" }
    Log To Console    Decrypt Request Body: ${decrypt_body}

    ${decrypt_response}=    Post Request     mysession     https://svcint.digitap.ai/validation/v1/decrypt   json=${decrypt_body}    headers=${headers}
    ...
    ${encrypt_body}=   Evaluate    { "plain_payload": { "request_id": "${encrypted_data_output1}" }, "secret_name": "crypto_framework_secret", "secret_key": "digitap_public", "kms_id": "39341d7d-b0d2-4c0a-82a3-1dacfac47d92", "encryption_type": "jwe" }
    ${headers}=    Create Dictionary    Content-Type=application/json
    Log To Console    Encrypt Request Body: ${encrypt_body}
